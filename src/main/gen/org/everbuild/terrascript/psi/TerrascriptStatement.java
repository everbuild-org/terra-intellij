// This is a generated file. Not intended for manual editing.
package org.everbuild.terrascript.psi;

import java.util.List;
import org.jetbrains.annotations.*;
import com.intellij.psi.PsiElement;

public interface TerrascriptStatement extends PsiElement {

  @Nullable
  TerrascriptConditional getConditional();

  @Nullable
  TerrascriptControlFlow getControlFlow();

  @Nullable
  TerrascriptExpression getExpression();

  @Nullable
  TerrascriptForLoop getForLoop();

  @Nullable
  TerrascriptIdAssignment getIdAssignment();

  @Nullable
  TerrascriptVariableDeclaration getVariableDeclaration();

  @Nullable
  TerrascriptWhileLoop getWhileLoop();

  @Nullable
  PsiElement getComment();

}
